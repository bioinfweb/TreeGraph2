...
public class MinLengthOperator extends AbstractBranchOperator implements FormatOperator {
  private DistanceValue length = null;

  
	public MinLengthOperator(DistanceValue length) {
		super();
		this.length = length;
	}
	...
	protected void doApplyTo(ElementFormats format) {
		((BranchFormats)format).getMinLength().assign(length);
	}
}