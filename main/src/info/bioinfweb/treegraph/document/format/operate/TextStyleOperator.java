...
public class TextStyleOperator extends AbstractTextOperator implements FormatOperator {
	private boolean isSet;
	private int style;

	
	public TextStyleOperator(boolean isSet, int style) {
		super();
		this.isSet = isSet;
		this.style = style;
	}
	...
	protected void doApplyTo(ElementFormats format) {
		if (isSet) {
			((TextFormats)format).addTextStyle(style);
		}
		else {
			((TextFormats)format).removeTextStyle(style);
		}
	}
}