 ...
 * @since 2.0.25
 */
public abstract class AbstractDistanceValueAdapter implements DistanceAdapter {
	protected abstract DistanceValue getDistanceValue(Node node);

	
	public float getDistance(Node node) {
		return getDistanceValue(node).getInMillimeters();
	}
	

	public void setDistance(float distance, Node node) {
		getDistanceValue(node).setInMillimeters(distance);
	}
}