...
public class FirstIndexOfFunction extends AbstractStringFunction {
	public FirstIndexOfFunction(CalculateColumnEdit edit) {
		super(edit);
	}
	...
	protected Object calculate(String text, Object[] additionalParameters) throws ParseException {
		try {
			if (additionalParameters.length == 2) {
				return text.indexOf(checkInteger(additionalParameters[0]), checkInteger(additionalParameters[1]));
			}
			else {
				return text.indexOf(checkInteger(additionalParameters[0]));
			}
		}
