...
public class LastIndexOfFunction extends AbstractStringFunction {
	public LastIndexOfFunction(CalculateColumnEdit edit) {
		super(edit);
	}
	...
	protected Object calculate(String text, Object[] additionalParameters) throws ParseException {
		try {
			if (additionalParameters.length == 2) {
				return text.lastIndexOf(checkInteger(additionalParameters[0]), checkInteger(additionalParameters[1]));
			}
			else {
				return text.lastIndexOf(checkInteger(additionalParameters[0]));
			}
		}
