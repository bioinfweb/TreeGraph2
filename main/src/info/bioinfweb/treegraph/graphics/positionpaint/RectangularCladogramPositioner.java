...
public class RectangularCladogramPositioner implements TreePositioner {
	...
	private float calculateWidthsHeights(Node root) {
	  ...
	  pd.setHeightAbove(0);  // wurzel.h�heOben = 0;
	  pd.setHeightBelow(0);  // wurzel.h�heUnten = 0;
	  float branchHeight = 0;
	  if (root.hasAfferentBranch()) { 	// wenn (zuf�hrender Ast vorhanden) dann
			...
			if (root.hasParent()) {
				...
				//TODO Wird die Dicke des Elternknotens schon wonanders ber�cksichtigt?
			}
	    root.getAfferentBranch().getPosition(type).getHeight().setInMillimeters(branchHeight);  // wurzel.elternast.h�he = wurzel.elternast.formate.liniendicke;
	    
			calculateLabelBlockDimensions(root.getAfferentBranch().getLabels(), true);  // wurzel.h�heOben = (H�hen der oberen Labels bestimmen);
			...
			pd.setHeightBelow(labelBlockHeight(root.getAfferentBranch().getLabels(), false) + 
					...
					0.5f * branchHeight);  // wurzel.h�heUnten = (H�hen der unteren Labels bestimmen);  
	  }  // wenn_ende;
	  
		DistanceDimension d = calculateTaxonDimension(root);
	  if (root.getChildren().size() > 0) {  // wenn (Anzahl(Unter�ste) > 0) dann
	    ...
	    pd.setDifAbove(subnode.getPosition(type).getHeightAbove());  // wurzel.difOben = wurzel.ersterUnterast.h�heOben;
	    if (root.getChildren().size() > 1) {  // wenn (Anzahl(Unter�ste) > 1) dann
	      pd.getHeight().setInMillimeters(subnode.getPosition(type).getHeightBelow()); // wurzel.h�he = wurzel.ersterUnterast.h�heUnten;
	      for (int i = 1; i <= root.getChildren().size() - 2; i++) {  // f�r alle Unter�ste zwischen 1 und n-2  // wird ggf. gar nicht durchlaufen
	        ...
	        pd.getHeight().add(subnode.getPosition(type).getHeightAbove() + root.getChildren().get(i).getPosition(type).getHeightBelow()); // wurzel.h�he += wurzel.unterast[i].h�heOben + wurzel.unterast[i].h�heUnten;
				}  // f�r_ende
        ...
        subnode = root.getChildren().get(root.getChildren().size() - 1);
	      ...
	      pd.getHeight().add(subnode.getPosition(type).getHeightAbove());  // wurzel.h�he += wurzel.letzterUnterast.h�heOben;
	    }
	    else {  // sonst
	      pd.getHeight().setInMillimeters(root.getFormats().getLineWidth().getInMillimeters());  // wurzel.h�he = 0 + Liniendicke;
	    }  // wenn_ende
	    pd.setDifBelow(subnode.getPosition(type).getHeightBelow());  // wurzel.difUnten = wurzel.letzterUnterast.h�heUnten;
	    // subnode hat in beiden m�glichen den F�llen hier den richtigen Wert.
	  }
	  else {  // sonst
	    pd.getHeight().assign(d.getHeight());  // wurzel.h�he = (H�he von Wurzel als Blatt bestimmen)
	    pd.setDifAbove(0);
	    pd.setDifBelow(0);
	  }  // wenn_ende

	  float halfHeight = 0.5f * pd.getHeight().getInMillimeters() + pd.getDifAbove();  // H�heOben2 = 0,5 * wurzel.h�he + wurzel.difOben;
    if (pd.getHeightAbove() > halfHeight) {  // wenn (wurzel.h�heOben > H�heOben2) dann
      pd.setDifAbove(pd.getHeightAbove() - 0.5f * pd.getHeight().getInMillimeters());  // wurzel.difOben = wurzel.h�heOben - 0.5 * wurzel.h�he;
    }
    else {  // sonst
      pd.setHeightAbove(halfHeight);  // wurzel.h�heOben = H�heOben2;
    }  // wenn_ende

    halfHeight = 0.5f * pd.getHeight().getInMillimeters() + pd.getDifBelow();  // H�heUnten2 = 0,5 * wurzel.h�he + wurzel.h�heUnten;
    if (pd.getHeightBelow() > halfHeight) {  // wenn (wurzel.h�heUnten > H�heUnten2) dann
      pd.setDifBelow(pd.getHeightBelow() - 0.5f * pd.getHeight().getInMillimeters());  // wurzel.difUnten = wurzel.h�heUnten - 0.5 * wurzel.h�he;
    }
    else {  // sonst
      pd.setHeightBelow(halfHeight);  // wurzel.h�heUnten = H�heUnten2;
    }  // wenn_ende
	  ...
	  if (root.hasAfferentBranch()) {  // wenn (zuf�hrender Ast vorhanden) dann
	    //  (Breiten der Labels des zuf�hrenden Asts bestimmen);  // bereits oben erfolgt
	  	Branch b = root.getAfferentBranch();
	    b.getPosition(type).getWidth().setInMillimeters(
	    		Math.max(labelBlockWidth(b.getLabels()), b.getFormats().getMinLength().getInMillimeters()));  // ast.breite = max(labels.blockWidth, minBranchWidth);  // Hier ggf. gespeicherte Astl�nge ber�cksichtigen.
	...
	protected float calculateCornerRadiusShift(Node node, float y) {
		...
		return -(float)Math.sqrt(Math.pow(cornerRadius, 2) - Math.pow(yCircle, 2)) - 0.5f * node.getFormats().getLineWidth().getInMillimeters();  //In der Kurve wird bis zur Mitte der Astlinie gezeichnet um L�cken bei dicken Linien zu vermeiden.
	}
  ...
  private AbstractPaintableElement searchLabelBlock(Branch branch, float x, float y, 
  	...
  	if (y <= branch.getPosition(type).getTop().getInMillimeters()) {  // Gesuchtes Label kann h�chstens oben sein.
  		above = true;
  	}
