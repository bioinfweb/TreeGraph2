...
public class MoveSubtreeUpAction extends MoveSubtreeAction {
	...
	public void setEnabled(Document document, TreeSelection selection, NodeBranchDataAdapter tableAdapter) {
		...
		if (enabled) {
			enabled = !getSelectedNode(selection).isFirst();
		}
		setEnabled(enabled);
	}
	...
	protected void onActionPerformed(ActionEvent e, TreeInternalFrame frame) {
		frame.getDocument().executeEdit(new MoveSubtreeEdit(frame.getDocument(), 
				getSelectedNode(frame.getTreeViewPanel().getSelection()), false));
	}
}